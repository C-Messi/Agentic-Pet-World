...
export interface CanvasInspection {
  ...
  cat: (PixelBounds & { centroidX: number; centroidY: number; pixels: number }) | null;
  furniture: {
...
export async function inspectCanvas(page: Page): Promise<CanvasInspection> {
  return page.locator('.game-surface canvas').evaluate(async (canvas: HTMLCanvasElement) => {
    ...
    for (let pixel = 0, offset = 0; pixel < total; pixel += 1, offset += 4) {
      ...
      if (r !== first[0] || g !== first[1] || b !== first[2]) variedPixels += 1;
      if (r < 55 && g < 50 && b < 55) darkPixels += 1;
      if (r > 225 && g > 220 && b > 200) lightPixels += 1;
      const isBubbleFill = (
        (r > 245 && g > 235 && b >= 195 && b <= 225)
        || (r >= 210 && r <= 230 && g >= 225 && g <= 240 && b >= 210 && b <= 235)
      );
      if (isBubbleFill) {
        bubbleFillPixels += 1;
        masks.bubble[pixel] = 1;
      }
      if (r >= 40 && r <= 50 && g >= 32 && g <= 42 && b >= 35 && b <= 45) bubbleTextPixels += 1;
      if (r > 220 && g > 165 && g < 235 && b < 125) goldPixels += 1;
      if (r > 195 && g >= 65 && g <= 150 && b < 90) masks.cat[pixel] = 1;
      if (r >= 140 && r <= 205 && g >= 75 && g <= 140 && b >= 45 && b <= 100) masks.wall[pixel] = 1;
      if (r >= 75 && r <= 155 && g >= 95 && g <= 170 && b >= 45 && b <= 125) masks.rug[pixel] = 1;
      if (r >= 120 && r <= 205 && g >= 175 && g <= 235 && b >= 180 && b <= 245) masks.window[pixel] = 1;
      if (r >= 175 && r <= 235 && g >= 70 && g <= 145 && b >= 70 && b <= 145) masks.coral[pixel] = 1;
      if (pixel % 97 === 0) {
        hash ^= (r << 16) | (g << 8) | b;
    ...
    const components: Array<PixelBounds & { pixels: number; centroidX: number; centroidY: number }> = [];
    for (let start = 0; start < total; start += 1) {
      ...
      while (cursor < queue.length) {
        ...
        maxX = Math.max(maxX, x);
        maxY = Math.max(maxY, y);
        for (const next of [index - 1, index + 1, index - copy.width, index + copy.width]) {
          if (next < 0 || next >= total || visited[next] === 1 || masks.cat[next] !== 1) continue;
    ...
    const bubbleComponents: Array<PixelBounds & { pixels: number; textPixels: number }> = [];
    for (let start = 0; start < total; start += 1) {
      ...
      while (cursor < queue.length) {
        ...
        maxY = Math.max(maxY, y);
        for (const next of [index - 1, index + 1, index - copy.width, index + copy.width]) {
          if (next < 0 || next >= total || bubbleVisited[next] === 1 || masks.bubble[next] !== 1) continue;
          const nextX = next % copy.width;
          if (Math.abs(nextX - x) > 1) continue;
      ...
      if (count < 200 || width < 80 || width > 260 || height < 25 || height > 130) continue;
      let textPixels = 0;
      for (let y = minY; y <= maxY; y += 1) {
        for (let x = minX; x <= maxX; x += 1) {
          const offset = (y * copy.width + x) * 4;
          if ((pixels[offset] ?? 255) < 105 && (pixels[offset + 1] ?? 255) < 105 && (pixels[offset + 2] ?? 255) < 105) {
            textPixels += 1;
          }
        }
      }
      bubbleComponents.push({ pixels: count, minX, minY, maxX, maxY, textPixels });
    }
...
export function hasRenderedRoom(inspection: CanvasInspection): boolean {
  return inspection.dataUrlLength > 10_000
